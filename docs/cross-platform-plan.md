...
|---|---|---|---|
| 全局键盘监听 | `CGEventTap` + 无障碍授权 | `SetWindowsHookEx(WH_KEYBOARD_LL)` | Windows **免授权**；注意杀软可能误报低级钩子 |
...
键盘钩子可考虑现成 crate `rdev`（跨平台），但**需验证它只暴露 keycode、不泄露字符**以满足隐私不变量；若不满足则按平台直接调 Win32 / CGEventTap。
